...
class select_sale(QWidget):

    def __init__(self):
        super().__init__()
        self.ui = uic.loadUi("ui/sel1.ui")

        self.price_table = create_table(table=self.ui.price_table,
    ...
    def saledetail_load_data(self, rank):
        ...
        for idx, (
            ...
            nextIdx = self.ui.price_table.rowCount()
            self.ui.price_table.insertRow(nextIdx)
            self.ui.price_table.setItem(nextIdx, 0, item_rank)
            self.ui.price_table.setItem(nextIdx, 1, item_code)
            self.ui.price_table.setItem(nextIdx, 2, item_name)
            self.ui.price_table.setItem(nextIdx, 3, item_price)
            self.ui.price_table.setItem(nextIdx, 4, item_salecnt)
            self.ui.price_table.setItem(nextIdx, 5, item_supply_price)
            self.ui.price_table.setItem(nextIdx, 6, item_addtax)
            self.ui.price_table.setItem(nextIdx, 7, item_sale_price)
            self.ui.price_table.setItem(nextIdx, 8, item_marginrate)
            self.ui.price_table.setItem(nextIdx, 9, item_marginprice)
